...
#Ydata=np.zeros((lab_data.shape[0],48,53),dtype='f')
#i=0
#for x in lab_data:
#      tmp=np.reshape(np.array(rep.rep_PBSC(x[0], alpha, 4)),(48,53))
#      Ydata[i,:,:]=tmp
#      print(x[0],i)
#      i=i+1
...
model = modele().cuda() 
model.load_state_dict(torch.load('/home/mmhiri/word_spotting_PBSC/IAM/test/models/model_cnn_dict_PBSC.pth'))
...
for ep in range(1000):
    ...
    if s_val / nb_val < threshold_valid:
        ...
        torch.save(model.state_dict(),'/home/mmhiri/word_spotting_PBSC/IAM/test/models/model_cnn_dict_PBSC.pth')
        torch.save(model,'/home/mmhiri/word_spotting_PBSC/IAM/test/models/model_cnn_PBSC.pth')
    else:
        lr_par = lr_par / 1.05
        losses.append([s_train/nb_train,s_val/nb_val])

    with open("/home/mmhiri/word_spotting_PBSC/IAM/test/models/losses_cnn_PBSC.csv", 'w') as f:
