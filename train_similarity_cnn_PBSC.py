...
data_pos=np.float32(np.load('/home/mmhiri/word_spotting_PBSC/IAM/data/pos_similarity_data_cnn_PBCS.npy'))
data_pos_val=data_pos[int(0.9*data_pos.shape[0]):]
data_pos=data_pos[:int(0.9*data_pos.shape[0])]

data_neg=np.float32(np.load('/home/mmhiri/word_spotting_PBSC/IAM/data/neg_similarity_data_cnn_PBCS.npy'))  
...
class modele(nn.Module):
    ...
    def __init__(self):
        ...
        self.l1=nn.Conv2d(53,256,(1,1))
        self.b1=nn.BatchNorm2d(256)

        self.l2=nn.Conv2d(256,16,(1,1))
        self.b2=nn.BatchNorm2d(16)

        self.l3 = nn.Linear(36*16, 64)
...
for ep in range(1000):
    ...
    for i in range(int(data_poss.shape[0]/1024)):
        
        input=np.concatenate([data_poss[i*1024:(i+1)*1024], data_negs[i*1024:(i+1)*1024]],axis=0)
        input=torch.from_numpy(input).cuda()
        output = model(input)[:,0]
        output1,output2=output[:1024], output[1024:]
        target=torch.tensor([1]*1024).cuda()
        loss=F.margin_ranking_loss(output1, output2, target,margin=0.1)
    ...
    for i in range(int(data_pos_val.shape[0]/1024)):

        input=np.concatenate([data_pos_val[i*1024:(i+1)*1024], data_neg_val[i*1024:(i+1)*1024]],axis=0)
        input=torch.from_numpy(input).cuda()
        output = model(input)[:,0]
        output1,output2=output[:1024], output[1024:]
        target=torch.tensor([1]*1024).cuda()
    ...
    if s_val / nb_val < threshold_valid:
        ...
        torch.save(model.state_dict(),'/home/mmhiri/word_spotting_PBSC/IAM/test/models/model_similarity_dict_cnn_PBSC.pth')
    else:
        lr_par = lr_par / 1.05
        losses.append([s_train/nb_train,s_val/nb_val])

    with open("/home/mmhiri/word_spotting_PBSC/IAM/test/models/losses_similarity_cnn_PBSC.csv", 'w') as f:
